...
import { STUDENT_FAQ_SECTIONS } from "../../data/faqData";
...
type StudentFormState = {
  ...
  firstChoiceLang?: string;
  secondChoiceName?: string;
  secondChoiceLang?: string;
};

// STUDENT_FAQ_SECTIONS is now imported from "../../data/faqData"
...
export default function StudentDashboardPage() {
  ...
  // Resolve major IDs to names/languages for the form state
  useEffect(() => {
    if (profile && majors.length > 0 && !majorsLoading) {
      const uniMajorObj = majors.find((m) => m.id === profileForm.universityMajor || m.id === profile.major?.id);
      const firstChoiceObj = majors.find((m) => m.id === profileForm.firstChoiceId);
      const secondChoiceObj = majors.find((m) => m.id === profileForm.secondChoiceId);

      setProfileForm((prev) => {
        const nextState = { ...prev };
        let changed = false;

        if (uniMajorObj && prev.uniMajorName !== uniMajorObj.name) {
          nextState.uniMajorName = uniMajorObj.name;
          changed = true;
        }
        if (uniMajorObj && prev.uniMajorLang !== uniMajorObj.language) {
          nextState.uniMajorLang = uniMajorObj.language;
          changed = true;
        }
        if (firstChoiceObj && prev.firstChoiceName !== firstChoiceObj.name) {
          nextState.firstChoiceName = firstChoiceObj.name;
          changed = true;
        }
        if (firstChoiceObj && prev.firstChoiceLang !== firstChoiceObj.language) {
          nextState.firstChoiceLang = firstChoiceObj.language;
          changed = true;
        }
        if (secondChoiceObj && prev.secondChoiceName !== secondChoiceObj.name) {
          nextState.secondChoiceName = secondChoiceObj.name;
          changed = true;
        }
        if (secondChoiceObj && prev.secondChoiceLang !== secondChoiceObj.language) {
          nextState.secondChoiceLang = secondChoiceObj.language;
          changed = true;
        }
  ...
  }, [profile, majors, majorsLoading]);

  // Sync universityMajor ID
  useEffect(() => {
    if (!profileForm.uniMajorName) {
      if (profileForm.universityMajor !== "") {
        setProfileForm((prev) => ({ ...prev, universityMajor: "" }));
      }
      return;
    }
    const found = majors.find(
      (m) => m.name === profileForm.uniMajorName && m.language === (profileForm.uniMajorLang || "magyar")
    );
    const nextId = found ? String(found.id) : "";
    if (profileForm.universityMajor !== nextId) {
      setProfileForm((prev) => ({ ...prev, universityMajor: nextId }));
    }
  }, [profileForm.uniMajorName, profileForm.uniMajorLang, majors]);

  // Sync firstChoiceId
  useEffect(() => {
    if (!profileForm.firstChoiceName) {
      if (profileForm.firstChoiceId !== "") {
        setProfileForm((prev) => ({ ...prev, firstChoiceId: "" }));
      }
      return;
    }
    const found = majors.find(
      (m) => m.name === profileForm.firstChoiceName && m.language === (profileForm.firstChoiceLang || "magyar")
    );
    const nextId = found ? String(found.id) : "";
    if (profileForm.firstChoiceId !== nextId) {
      setProfileForm((prev) => ({ ...prev, firstChoiceId: nextId }));
    }
  }, [profileForm.firstChoiceName, profileForm.firstChoiceLang, majors]);

  // Sync secondChoiceId
  useEffect(() => {
    if (!profileForm.secondChoiceName) {
      if (profileForm.secondChoiceId !== "") {
        setProfileForm((prev) => ({ ...prev, secondChoiceId: "" }));
      }
      return;
    }
    const found = majors.find(
      (m) => m.name === profileForm.secondChoiceName && m.language === (profileForm.secondChoiceLang || "magyar")
    );
    const nextId = found ? String(found.id) : "";
    if (profileForm.secondChoiceId !== nextId) {
      setProfileForm((prev) => ({ ...prev, secondChoiceId: nextId }));
    }
  }, [profileForm.secondChoiceName, profileForm.secondChoiceLang, majors]);

  // Sync languages if not available for selected name
  useEffect(() => {
    if (!profileForm.uniMajorName) return;
    const availableLangs = getLanguagesForMajor(profileForm.uniMajorName);
    if (availableLangs.length > 0 && !availableLangs.includes(profileForm.uniMajorLang || "magyar")) {
      setProfileForm((prev) => ({ ...prev, uniMajorLang: availableLangs[0] }));
    }
  }, [profileForm.uniMajorName, majors]);

  useEffect(() => {
    if (!profileForm.firstChoiceName) return;
    const availableLangs = getLanguagesForMajor(profileForm.firstChoiceName);
    if (availableLangs.length > 0 && !availableLangs.includes(profileForm.firstChoiceLang || "magyar")) {
      setProfileForm((prev) => ({ ...prev, firstChoiceLang: availableLangs[0] }));
    }
  }, [profileForm.firstChoiceName, majors]);

  useEffect(() => {
    if (!profileForm.secondChoiceName) return;
    const availableLangs = getLanguagesForMajor(profileForm.secondChoiceName);
    if (availableLangs.length > 0 && !availableLangs.includes(profileForm.secondChoiceLang || "magyar")) {
      setProfileForm((prev) => ({ ...prev, secondChoiceLang: availableLangs[0] }));
    }
  }, [profileForm.secondChoiceName, majors]);
                ...
                {!profileLoading && profile && (
                  <div className="flex items-center justify-between rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-4 transition-colors">
                    ...
                    <button
                      ...
                      type="button"
                      className={`relative inline-flex h-7 w-12 items-center rounded-full transition-colors focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 dark:focus:ring-offset-slate-900 ${profile.isAvailableForWork
                        ? "bg-blue-600 border-blue-600 dark:bg-blue-500"
                        : "bg-slate-200 dark:bg-slate-700"
                        }`}
                    >
                      <span
                        className={`inline-block h-5 w-5 transform rounded-full bg-white transition-transform duration-200 ease-in-out ${profile.isAvailableForWork
                          ? "translate-x-6"
                          : "translate-x-1"
                          }`}
                      />
                      ...
                      {profileForm.studentType === "HIGHSCHOOL" ? (
                        <div className="space-y-4">
                          <div className="space-y-1">
                            ...
                            <select
                              value={profileForm.firstChoiceName ?? ""}
                              onChange={(e) =>
                                setProfileForm((p) => ({
                                  ...p,
                                  firstChoiceName: e.target.value,
                                }))
                              }
                          ...
                          <div className="space-y-1">
                            ...
                            <select
                              value={profileForm.firstChoiceLang ?? "magyar"}
                              onChange={(e) =>
                                setProfileForm((p) => ({
                                  ...p,
                                  firstChoiceLang: e.target.value,
                                }))
                              }
                              disabled={!profileForm.firstChoiceName || majorsLoading}
                              className="w-full rounded-lg border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-900 px-3 py-2 text-sm text-slate-900 dark:text-slate-100 focus:outline-none focus:ring-2 focus:ring-blue-500 disabled:bg-slate-100 dark:disabled:bg-slate-800 transition-colors"
                            >
                              {getLanguagesForMajor(profileForm.firstChoiceName || "").map((lang) => (
                          ...
                          <div className="space-y-1">
                            ...
                            <select
                              value={profileForm.secondChoiceName ?? ""}
                              onChange={(e) =>
                                setProfileForm((p) => ({
                                  ...p,
                                  secondChoiceName: e.target.value,
                                }))
                              }
                              className="w-full rounded-lg border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-900 px-3 py-2 text-sm text-slate-900 dark:text-slate-100 focus:outline-none focus:ring-2 focus:ring-blue-500 transition-colors"
                            >
                              ...
                              {uniqueMajorNames.map((name) => (
                                <option
                                  key={name}
                                  value={name}
                                  disabled={name === profileForm.firstChoiceName && getLanguagesForMajor(name).length === 1}
                                >
                                  {name}
                                </option>
                              ))}
                          ...
                          <div className="space-y-1">
                            ...
                            <select
                              value={profileForm.secondChoiceLang ?? "magyar"}
                              onChange={(e) =>
                                setProfileForm((p) => ({
                                  ...p,
                                  secondChoiceLang: e.target.value,
                                }))
                              }
                              disabled={!profileForm.secondChoiceName || majorsLoading}
                              className="w-full rounded-lg border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-900 px-3 py-2 text-sm text-slate-900 dark:text-slate-100 focus:outline-none focus:ring-2 focus:ring-blue-500 disabled:bg-slate-100 dark:disabled:bg-slate-800 transition-colors"
                            >
                              {getLanguagesForMajor(profileForm.secondChoiceName || "").map((lang) => (
                                <option
                                  key={lang}
                                  value={lang}
                                  disabled={profileForm.secondChoiceName === profileForm.firstChoiceName && lang === profileForm.firstChoiceLang}
                                >
                                  {lang}
                                </option>
                              ))}
                        ...
                        <div className="space-y-4">
                          <div className="space-y-1">
                            ...
                            <select
                              value={profileForm.uniMajorName ?? ""}
                              onChange={(e) =>
                                setProfileForm((p) => ({
                                  ...p,
                                  uniMajorName: e.target.value,
                                }))
                              }
                              className="w-full rounded-lg border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-900 px-3 py-2 text-sm text-slate-900 dark:text-slate-100 focus:outline-none focus:ring-2 focus:ring-blue-500 transition-colors"
                            >
                          ...
                          <div className="space-y-1">
                            ...
                            <select
                              value={profileForm.uniMajorLang ?? "magyar"}
                              onChange={(e) =>
                                setProfileForm((p) => ({
                                  ...p,
                                  uniMajorLang: e.target.value,
                                }))
                              }
                              disabled={!profileForm.uniMajorName || majorsLoading}
                              className="w-full rounded-lg border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-900 px-3 py-2 text-sm text-slate-900 dark:text-slate-100 focus:outline-none focus:ring-2 focus:ring-blue-500 disabled:bg-slate-100 dark:disabled:bg-slate-800 transition-colors"
                            >
                              {getLanguagesForMajor(profileForm.uniMajorName || "").map((lang) => (
                                <option key={lang} value={lang}>
                                  {lang}
                                </option>
                              ))}
            ...
            {activeTab === "guide" && (
              <div className="space-y-6">
                ...
                <div className="rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-950 p-6 shadow-sm dark:shadow-none transition-colors">
                  ...
                  <p className="text-sm text-slate-600 dark:text-slate-400 mb-5 transition-colors">
                    Gyakran ismételt kérdések a platform használatához és a képzés folyamatához.
                  </p>
