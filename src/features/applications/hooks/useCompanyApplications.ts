...
export const useCompanyApplications = () => {
  ...
  const [sortBy, setSortBy] = useState<
    "newest" | "oldest" | "student-asc" | "student-desc" | "position-asc"
  >("newest");
    ...
    try {
      ...
      const message =
        err instanceof Error
          ? err.message
          : "Hiba a jelentkezések betöltésekor.";
      setError(message);
    } finally {
      setLoading(false);
    }
  ...
  const handleApplicationDecision = async (
    ...
    try {
      ...
      const message =
        err instanceof Error
          ? err.message
          : "Hiba a jelentkezés frissítésekor.";
    ...
    if (searchQuery.trim()) {
      ...
      filtered = filtered.filter(
        (app) =>
          ...
          (
            app.student?.studentProfile?.currentMajor?.toLowerCase() || ""
