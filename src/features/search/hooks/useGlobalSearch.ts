...
export function useGlobalSearch() {
  ...
  const staticPages: SearchResult[] = useMemo(() => {
    const pages: SearchResult[] = [
      { id: 'home', type: 'page', title: 'Kezdőlap', link: '/', subtitle: 'Főoldal' },
      { id: 'jobs', type: 'page', title: 'Elérhető állások', link: '/positions', subtitle: 'Minden aktuális állásajánlat' },
    ];

    if (isAuthenticated) {
        if (dashboardLink) {
            pages.push({ id: 'dashboard', type: 'page', title: 'Irányítópult', link: dashboardLink, subtitle: 'Saját vezérlőpult' });
        }
    } else {
        pages.push({ id: 'login', type: 'page', title: 'Bejelentkezés', link: '/login', subtitle: 'Belépés a fiókba' });
        pages.push({ id: 'register', type: 'page', title: 'Regisztráció', link: '/register', subtitle: 'Új fiók létrehozása' });
    }
    
    if (newsLink) {
        pages.push({ id: 'news', type: 'page', title: 'Hírek', link: newsLink, subtitle: 'Aktuális hírek és információk' });
    }
    ...
    const timer = setTimeout(async () => {
      ...
      try {
        ...
        const [positions, companies, news] = await Promise.all([
          api.positions.listPublic({ limit: 50 }).then(res => res || []).catch(() => []),
          api.companies.list({ limit: 50 }).then(res => res || []).catch(() => []),
           ...
           api.news.list({ limit: 20 }).then(res => res || []).catch(() => [])
        ]);
        ...
        setResults([...pageResults, ...positionResults, ...companyResults, ...newsResults]);
