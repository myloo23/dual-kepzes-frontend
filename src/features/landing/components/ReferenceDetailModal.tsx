...
export default function ReferenceDetailModal({ person, isOpen, onClose }: ReferenceDetailModalProps) {
        ...
        >
            <div className="flex flex-col md:flex-row h-full">
                ...
                <div className="w-full md:w-2/5 bg-slate-50 border-r border-slate-100 p-8 flex flex-col items-center text-center">
                    <div className={`h-40 w-40 rounded-full flex-shrink-0 flex items-center justify-center font-bold text-4xl shadow-lg border-4 border-white mb-6 overflow-hidden ${!person.image ? getRandomColor(person.name) : 'bg-white'}`}>
                        {person.image ? (
                            <img 
                                src={person.image} 
                                alt={person.name} 
                    ...
                    <div className="w-full space-y-3 text-left">
                        ...
                        <div className="bg-white p-3 rounded-xl border border-slate-100 shadow-sm hover:border-dkk-blue/30 transition-colors group">
                            <div className="flex items-center gap-3">
                                <Mail size={18} className="text-dkk-blue group-hover:scale-110 transition-transform" />
                                <a
                        ...
                        {person.description ? (
                            person.description.split('\n').filter(line => line.trim() !== '').map((line, idx) => {
                                ...
                                if (line.trim().startsWith('- ') || line.trim().startsWith('• ')) {
                                    return (
                                        <div key={idx} className="flex items-start gap-2 mb-2 pl-4">
                                ...
                                if (line.trim().endsWith(':')) {
                                    return (
                                        <p key={idx} className="mb-2 font-bold text-slate-800 mt-4">
                                            {line}
                                        </p>
                                    );
                                }
