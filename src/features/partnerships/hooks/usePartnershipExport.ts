...
export const usePartnershipExport = () => {
  const handleExport = useCallback((partnerships: Partnership[]) => {
    ...
    const flatData = partnerships.map(p => ({
      ...
      status: p.status,
      semester: p.semester || '-',
      startDate: p.startDate ? new Date(p.startDate).toLocaleDateString('hu-HU') : '-',
      endDate: p.endDate ? new Date(p.endDate).toLocaleDateString('hu-HU') : '-'
    }));

    exportToExcel(flatData, getExportFilename('partnerships', 'xlsx'), columns as any);
  }, []);
