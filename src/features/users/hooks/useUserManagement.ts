...
export function useUserManagement(
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const errorMsg =
        e instanceof Error ? e.message : ERROR_MESSAGES.FETCH_USERS;
      setError(errorMsg);
    } finally {
      setLoading(false);
    }
  ...
  const getById = useCallback(
    async (id: Id): Promise<UserItem | null> => {
      if (!id) return null;
      ...
      try {
        ...
        const errorMsg =
          e instanceof Error ? e.message : ERROR_MESSAGES.USER_NOT_FOUND;
        setError(errorMsg);
        return null;
      } finally {
        setLoading(false);
  ...
  const deleteUser = useCallback(
    async (id: Id): Promise<boolean> => {
      ...
      try {
        ...
        const errorMsg =
          e instanceof Error ? e.message : ERROR_MESSAGES.DELETE_FAILED;
        setError(errorMsg);
        return false;
      } finally {
        setLoading(false);
  ...
  const reactivateUser = useCallback(
    async (id: Id): Promise<boolean> => {
      ...
      try {
        ...
        const errorMsg =
          e instanceof Error ? e.message : ERROR_MESSAGES.GENERIC;
        setError(errorMsg);
        return false;
      } finally {
        setLoading(false);
  ...
  const updateStudent = useCallback(
    async (id: Id, data: Partial<StudentProfile>): Promise<boolean> => {
      try {
        ...
        const errorMsg =
          e instanceof Error ? e.message : ERROR_MESSAGES.UPDATE_FAILED;
        setError(errorMsg);
        return false;
      }
    },
