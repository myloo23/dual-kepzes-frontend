...
export function NotificationList({ notifications, onMarkRead, onArchive, onDelete, isLoading }: NotificationListProps) {
  if (isLoading) {
    return (
      <div className="space-y-4">
        {[1, 2, 3].map((i) => (
          <div key={i} className="animate-pulse bg-white rounded-xl border border-slate-200 p-6 h-32" />
        ))}
      </div>
    );
  }
  ...
  if (notifications.length === 0) {
    return (
      <div className="text-center py-20 bg-white rounded-xl border border-slate-200">
        ...
        <h3 className="text-lg font-medium text-slate-900">Nincs megjeleníthető értesítés</h3>
        <p className="text-slate-500 mt-1">Jelenleg nincs az új üzeneted a kiválasztott szűrők alapján.</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-4">
      {notifications.map((notification) => {
        ...
        const roleClass = ROLE_COLORS[notification.senderRole || ''] || 'bg-slate-100 text-slate-600 border-slate-200';
        const typeLabel = TYPE_LABELS[notification.type || ''] || notification.type || 'Egyéb';
        ...
        return (
          <div 
            ...
            className={cn(
              ...
              isRead ? "border-slate-200" : "border-blue-200 bg-blue-50/10 shadow-sm"
            )}
          >
            <div className="flex items-start justify-between gap-4">
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-3 mb-2">
                  {roleLabel && (
                    <span className={cn("px-2.5 py-0.5 rounded-full text-xs font-semibold border", roleClass)}>
                      {roleLabel}
                    </span>
                  )}
                  ...
                  <span className="text-xs text-slate-400">
                    {notification.createdAt ? new Date(notification.createdAt).toLocaleDateString('hu-HU', { 
                ...
                <h4 className={cn("text-base font-semibold mb-1", isRead ? "text-slate-900" : "text-blue-900")}>
